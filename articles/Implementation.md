...
applicable ([Bank 2013](#ref-worldbank2013); [European Union/OECD
...
been used by Istat, Italy’s National Statistical Office, that produces
experimental subnational PPP statistics ([Istat 2026](#ref-Istat)).

Upon validation of the raw data,[^1] the approach follows a two-step
procedure ([Figure 1](#fig-aggregation)):
...
1.  Estimation of price parities at the basic-heading level using the
    ...
    ([Summers 1973](#ref-summers1973international)). Basic-heading
    aggregation with CPD is recommended by the ICP ([ICP
    2021](#ref-icp2021)) as it is better-suited to handling missing
    price observations in the underlying price microdata than GEKS
    ([Auer, Ludwig von 2026](#ref-auer_von_gap_2026)).[^2]

2.  Aggregation of BH-level parities into higher-level indices using the
    Gini-Éltetö-Köves-Szulc (GEKS) method, a multilateral index
    construction technique that ensures transitivity, in combination
    with household final consumption expenditure data as a weighting
    structure for household final consumption PPPs ([ICP
    2021](#ref-icp2021)).

![](figures/cpd_geks.png)

Figure 1: Estimation and aggregation steps of subnational purchasing
power parities

To seamlessly combine the two estimation steps, `OECDsppps` provides the
option to impute missing basic-heading subnational PPPs. A complete
stylised workflow is described in section [Putting it all
...
data-specific, and is typically the most time-consuming part of the
initial data work, as data can be available at different levels of
granularity (spatial and product-related), content (available variables
and information), and coverage (e.g., products, types of activity,
etc.).
...
[Table 2](#tbl-example) for a stylised example based on ([Weinand and
Auer 2020](#ref-weinand2020)).
...
([2020](#ref-weinand2020))

In addition to harmonising the individual price quotes, initial data
processing also classifies the individual items or products according to
...
involves outlier detection of single price quotes and average price
...
The first step is the [estimation of basic headings using item-level
prices](https://amannj.github.io/OECDsppps/articles/Estimation.html#sec-step1).
There, price data are aggregated up to the level of basic headings,
generally without the use of expenditure weights.
...
method is recommended for aggregating above the basic-heading level for
...
[^2]: This is particularly important when deriving PPPs from existing
