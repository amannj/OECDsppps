...
specification, in which the region effects yield estimates of subnational
...
data by setting the `weights` argument to `'raw'`; see `estim_cpd()` for more
information.
...
Note that the `reframe()` function, from package `dplyr` [@refdplyr], in the code chunk 
