...
2.  [Estimation of higher level aggregates using basic heading indices](#sec-step2) 
    to higher levels of the classification
...
'linking function' `estim_index_link()` to impute missing basic-heading 
subnational PPPs. A complete sylised workflow is described in section
[Putting it all together](#sec-combined).
...
recently used in @hearne2025 and is publicly available:

-   `uk_cpi()` is a snipped of the [UK CPI
...
identical to the bahaviour of `cpd()` in `pricelevels`
...
This will return a grouped tibble containing three columns: the variable specified
by `group_by` (in this case the basic heading "group"); region ("region"),
and the subnational PPPs estimated via the CPD method ("sPPP"). 

Note that the returned tibble is still grouped by the grouping variable 
specified in `group_by()`, which should be resolved with `ungroup()` before proceeding further.

## Index calculation using basic heading indices

GEKS (as well as Laspeyres, Paasche, Fisher, which are all nested within the GEKS function) indices can be calculated by
linking the previous commands with `index_geks()` (as well as `index_laspeyres()`,
`index_paasche()`, `index_fisher()`) through the
`estim_index_link()` function.

`estim_index_link()` takes tibble (data frame) containing the basic heading
sPPPs, and joins it with a supplied data frame containing the household
expenditure weights, and gives the user the option to impute price quotes for
expenditure categories, for which household expenditure weights are present but 
nor price quotes are available.

Thus, `estim_index_link()`  returns a data object with all necessary 
variables for the calculation of the price indices, e.g., `index_geks()`.
...
Note that in the code chunk below, `tryCatch()` is used to print the error message
returned by `index_geks()` as an output.
