...
dt1 <- pricelevels::rdata(R = R, B = B, N = N,
                          weights = ~r+n,
                          settings = list(par.sd = c(lnP = 0.1, 
                                                     pi = exp(1), 
                                                     delta = 0.5, 
                                                     error = 0.8))
) %>%
...
This yields a data frame containing the basic heading ("group",
inherited from the original item-level price data), region ("region"),
and sPPP ("sPPP"). The data frame is grouped by the basic heading and
should be ungrouped with `ungroup()` before proceeding further.
...
Laspeyres, Paasche, Fisher, and GEKS indices can be calculated by
linking the previous commands with `index_laspeyres()`,
`index_paasche()`, `index_fisher()`, and `index_geks()` through the
`estim_index_link()` function.

`estim_index_link()` takes a data frame containing the basic heading
sPPPs, and joins it with a supplied data frame containing the
expenditure weights. Additionally, it renames all variables so that its
output can be linked directly into the index functions, e.g.,
`index_geks()`.
...
Note that the auxiliary function `read_returns` simply returns the error message
returned by `index_geks()` as an output, rather than an error mesage which stops
the execution of the script.

```{r}
read_returns <- function(expr){
  tryCatch(expr,
           error = function(e){
...
We do not provide an option to impute expenditure weights, as we do for
missing sPPPs. In this instance, further analysis of the reason for
missing expenditure weights is required.
