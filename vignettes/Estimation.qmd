...
The function `estim_cpd()` also has the option to export extended regression output of the CPD model with argument `output = "Full"`, which summarises the key information of the estimate CPD model in a tidy `tibble()` using  \pkg{broom}. Information in the extended regression output is used to support the validation of CPD-based subnational PPPs at the basic-heading level; see [Validation](https://amannj.github.io/OECDsPPPs/articles/Validation.html#sec-tobh) vignette.
