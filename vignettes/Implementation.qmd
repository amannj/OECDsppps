...
subnational PPPs by the ICP [@icp2021], and has also been used by Istat, 
Italy's National Statistical Office, that produces
experimental subnational PPP statistics [@Istat]. 

Upon validation of the raw data,^[See section [Validation](https://amannj.github.io/OECDsppps/articles/Validation.html) of this package.] the approach follows a two-step procedure (@fig-aggregation): 

1. Estimation of price parities at the
basic-heading level using the regional extension of the
Country-Product-Dummy (CPD) method [@summers1973international]. Basic-heading
...
price microdata than GEKS [@auergap].^[This is particularly important when deriving PPPs from existing price microdata, where not all products are observed in every region, rather than from dedicated PPP surveys based on a pre-specified list of comparable items. In particular, GEKS price-level estimates are intrinsically more sensitive to missing observations than those obtained through CPD and therefore diagnostic tools for assessing data completeness are especially valuable for GEKS-based price-level estimates  [@auergap].]

2. Aggregation of BH-level parities into higher-level
...
![Estimation and aggregation steps of subnational purchasing power
...
A complete stylised workflow is described in section
...
| **2** Raw data validation | OECD or country | `valid_pot()`, `valid_apt()`, `valid_ratio_xr()`, `valid_ratio_ppp()`,  `valid_est()`, `valid_gpc()`, `valid_gpt()` |
...
data-specific, and is typically the most time-consuming part of the initial
...
@tbl-example for a stylised example based on [@weinand2020].
...
: Example of consumer price microdata based on @weinand2020 {#tbl-example}
...
The **first step of the raw data validation** is to assess the completeness of the underlying item-level price microdata, as systematically missing price quotes may bias the estimation of subnational PPPs.^[This assessment is particularly important when deriving PPPs from existing price microdata, where products are not necessarily observed in every region, rather than from dedicated PPP surveys based on a predefined list of comparable items. In particular, GEKS estimates are inherently more sensitive to missing observations than CPD estimates, making diagnostic tools for assessing data completeness especially valuable when applying the GEKS methodology [@auergap].]

Functions used at this stage are: `valid_gpc()` and `valid_cpt()`, and are described in 
are described in more detail in the [Validation
vignette](https://amannj.github.io/OECDsppps/articles/Validation.html#sec-gappattern).

The **second step of the raw data validation** covers the analysis of item-level prices within regions and
...
In case alternative data sources are used, the correspondig raw data [validation of alternative data
...
`valid_ratio_xr()`, `valid_ratio_ppp()`, `valid_est()`, and are described in 
are described in more detail in the [Validation
vignette](https://amannj.github.io/OECDsppps/articles/Validation.html).
...
[estimation of basic headings using item-level prices](https://amannj.github.io/OECDsppps/articles/Estimation.html#sec-step1).
There, price data are aggregated up to the level
of basic headings, generally without the use of expenditure weights.

This first estimation step is carried out using `estim_cpd()` with argument
`output = "Full"`, which summarises the key information of the estimated
...
Function `estim_index_link()` serves as a "link" from the first CPD steps towards the second GEKS step of the index calculation.
It combines the CPD estimates with the corresponding household expenditure weights (typically at the basic-heading level corresponding to the 4-digit COICOP classes) and identifies missing CPD-price-household-weight pairs.
It also provides the option to fill in missing basic-heading PPPs with a value given by the user,
thereby producing a complete CPD-price-household-weight matrix, which is needed for the second GEKS aggregation step.^[Function `estim_index_link()` only provides an option to impute missing CPD price estimates for existing household weights. The alternative scenario, where CPD estimates exist for categories with non-existing household weights is not considered.]
